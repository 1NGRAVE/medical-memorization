...
export default function App() {
  ...
  const handleTestConnection = useCallback(async (): Promise<boolean> => {
    if (settings.providerType === 'gemini' && settings.apiKey) {
      return createGeminiProvider(settings.apiKey).testConnection(settings.apiKey)
    }
    return true
  }, [settings.providerType, settings.apiKey])
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-white">
          ...
          <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
            ...
            <button
              ...
              className={`px-3 py-1.5 text-xs rounded-lg transition-all ${
                showConfig ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
              }`}
            >
              ⚙️ 模型
            </button>
        ...
        <footer className="text-center py-6 text-xs text-gray-400">
          牙科知识 AI 问答 · 判分：{settings.providerType === 'mock' ? '本地' : 'Gemini'}
        </footer>
      </div>
    )
  }
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-white">
        ...
        <main className="px-4 py-6">
          <ImportPanel
            ...
            apiKey={settings.providerType === 'gemini' ? settings.apiKey : undefined}
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-white flex items-center justify-center p-4">
        <div className="max-w-lg w-full bg-white rounded-3xl shadow-xl border border-gray-100 p-8 text-center space-y-6">
          ...
          <button onClick={handleBackToDecks} className="w-full py-2.5 bg-gray-100 text-gray-600 rounded-xl text-sm hover:bg-gray-200 transition-all">
            ← 返回题库列表
          </button>
        </div>
      </div>
      ...
      <main className="max-w-2xl mx-auto px-4 py-6">
        ...
        {loadingCards ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 mb-3">该题库还没有卡片</p>
            <button
            ...
            {session.results.length > 0 && (
              <div className="flex gap-3 mb-6">
                ...
                <div className="flex-1 bg-white rounded-xl border border-gray-100 px-4 py-2 flex items-center justify-between">
                  ...
                  <span className="text-xs font-medium text-gray-700">{settings.providerType === 'mock' ? '本地' : 'Gemini'}</span>
                </div>
              </div>
            )}
            {currentCard && (
